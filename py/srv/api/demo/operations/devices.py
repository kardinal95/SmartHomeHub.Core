...
def add_devices(devices, session):
    for num, item in enumerate(devices):
        try:
            add_device(item=item, session=session)
...
def add_device(item, session):
    ...
    for parameter in model.parameters:
        device.parameters.append(DeviceParameterMdl(device_param=parameter.parameter,
                                                    value=parameter.value,
                                                    value_type=type(parameter.value).__name__))
